                    ...
                    <div class="row p-2">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-5">
                                    <div class="input-group">
                                      <input type="text" class="form-control" id="bukti" name="bukti" maxlength="10" value="{{$kasm->bukti}}" readonly>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-control" id="kas" name="kas">
                                        ...
                                        <option value="{{$kl->id}}"  @if($kl->id==$kasm->kas) selected="selected" @endif>{{$kl->nama}}</option>
                                        @endforeach
                                    </select>    
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-control" id="relasi" name="relasi">
                                        ...
                                        <option value="{{$rl->id}}" @if($rl->id==$kasm->relasi) selected="selected" @endif>{{$rl->nama}}</option>
                                        @endforeach
                                    </select>   
                                </div>
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group row">
                                ...
                                <div class="col-md-4">
                                    <input type="text" class="form-control date-picker" data-date-format="dd-mm-yyyy" id="tgl" name="tgl" maxlength="12" value="{{FormatTgl($kasm->tgl)}}">
                                </div>
                            </div>
                            <div class="form-group row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <input type="text" class="form-control" id="ket" name="ket" value="{{ $kasm->ket }}" maxlength="255">
                                </div>
                            </div>
                        </div>
                    </div>
...
@push('js')
    ...
    $('#nilai').keypress(function(event) {
        ...
        $('#form_trans').submit(function(e) {
            ...
            $.ajax({
                type:'post',
                url: "{{ route('kasm.update', $kasm->bukti) }}",
                data: formData,
                cache:false,
                contentType: false,
                processData: false,
                ...
                success: function(response){
                    if(response["success"]){
                        if(termasuk_detail=='Y'){
                            simpan_detail();
                        } else window.location.replace("{{ url('admin/kasm')}}");    
                    } else  alert('Gagal');
                },
                error:function(data){
   	...
   	`` }
        ...
        $('#form_detail').submit(function(e) {
            ...
            var id= $("#id").val();
            var link='';
            if(jns=='i') link="{{ url('admin/simpan_kasmd') }}";
            else link="{{ url('admin/update_kasmd') }}/"+id;
            ...
            $.ajax({
                ...
                success: function(response){
                    if(response["success"]){
                        $("#Modal").modal('hide');
                        $("#detail").load("{{url('admin/tampilkan_kasmd/'.$kasm->bukti)}}");
